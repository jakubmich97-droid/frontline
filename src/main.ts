...
function mapMarkup() {
  ...
  return `<svg class="world-map" viewBox="0 0 1350 1150" role="group" aria-label="Strategická mapa, ${playableRegions(game).length} obyvatelných provincií"><defs><linearGradient id="ocean" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#1b4a59"/><stop offset=".5" stop-color="#103444"/><stop offset="1" stop-color="#082536"/></linearGradient><radialGradient id="ocean-light"><stop offset="0" stop-color="#70a8aa" stop-opacity=".24"/><stop offset="1" stop-color="#102c3c" stop-opacity="0"/></radialGradient><pattern id="grid" width="64" height="64" patternUnits="userSpaceOnUse"><path d="M64 0H0V64" fill="none" stroke="#b0d0cd" stroke-opacity=".07" stroke-width=".7"/></pattern><pattern id="contours" width="180" height="110" patternUnits="userSpaceOnUse"><path d="M-20 72C32 25 92 118 205 42M-15 91C48 48 116 137 204 69" fill="none" stroke="#d0e1d7" stroke-opacity=".065" stroke-width="1.2"/></pattern><filter id="land-shadow" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dx="0" dy="5" stdDeviation="6" flood-color="#02090d" flood-opacity=".72"/></filter><filter id="soft-glow"><feGaussianBlur stdDeviation="8"/></filter>${gradients}</defs><rect width="1350" height="1150" fill="url(#ocean)"/><rect width="1350" height="1150" fill="url(#ocean-light)"/><ellipse cx="1060" cy="980" rx="260" ry="120" fill="#5e9b9a" opacity=".08" filter="url(#soft-glow)"/><rect width="1350" height="1150" fill="url(#contours)"/><rect width="1350" height="1150" fill="url(#grid)"/><path class="sea-current" d="M15 210C210 150 360 240 510 175M840 760C1010 690 1150 790 1340 710M20 625C180 570 245 640 350 600M120 1035C330 970 490 1080 690 1010"/><text x="28" y="27" class="map-note">JANTAROVÁ EVROPA / KOMODITNÍ MAPA</text><text x="1320" y="27" class="map-note">N ↑</text><text x="118" y="450" text-anchor="middle" class="sea-label">ZÁPADNÍ MOŘE</text><text x="675" y="1124" text-anchor="middle" class="sea-label">JIŽNÍ MOŘE</text>
${game.regions
  .map((r) => {
    ...
    return `<g class="region terrain-${r.terrain} ${selected === r.id ? "selected" : ""} ${target === r.id ? "targeted" : ""} ${battle ? "contested" : ""}" role="button" tabindex="0" data-region="${r.id}" aria-label="${r.name}, ${f.name}, úroveň ${r.level}, ${r.owner < 0 ? "neutrální posádka " + num(defenseStrength(game, r)) : game.nations[r.owner].name}" aria-pressed="${selected === r.id}"><polygon points="${r.polygon}" fill="${fill}" fill-opacity="${battle ? ".62" : r.owner === game.player ? ".5" : r.owner < 0 ? ".22" : ".44"}" stroke="${color}" stroke-opacity="${r.owner < 0 ? ".55" : ".2"}"/><foreignObject x="${r.x - 15}" y="${r.y - 44}" width="30" height="30" class="facility-symbol"><div xmlns="http://www.w3.org/1999/xhtml" class="map-sprite sprite-${r.facility}"></div></foreignObject><text x="${r.x}" y="${r.y + 1}" class="region-name">${r.name}</text><text x="${r.x}" y="${r.y + 18}" class="facility-label" fill="${color}">${f.short} · ${r.level}</text>${supply}${r.owner < 0 && !battle ? `<g class="neutral-garrison"><rect x="${r.x - 27}" y="${r.y + 25}" width="54" height="18" rx="9"/><text x="${r.x}" y="${r.y + 38}">⬟ ${num(defenseStrength(game, r))}</text></g>` : r.fort > 0 ? `<text x="${r.x}" y="${r.y + 36}" class="fort-label">⬟ ${r.fort}</text>` : ""}${r.upgrade ? `<text x="${r.x}" y="${r.y + 52}" class="map-timer upgrade-timer">↑ ${r.upgrade.remaining} s</text>` : ""}${r.fortification ? `<text x="${r.x}" y="${r.y + (r.upgrade ? 65 : 52)}" class="map-timer fort-timer">⬟ ${r.fortification.remaining} s</text>` : ""}${battle ? `<g class="battle-progress"><rect x="${r.x - 34}" y="${r.y + 28}" width="68" height="10" rx="5"/><rect class="progress-fill" x="${r.x - 32}" y="${r.y + 30}" width="${Math.max(1, battle.progress * 64)}" height="6" rx="3" fill="${game.nations[battle.owner].color}"/><text x="${r.x}" y="${r.y + 50}">${Math.round(battle.progress * 100)} %</text></g>` : ""}</g>`;
  })
  .join("")}
...
function render() {
  const previousPanel = document.querySelector<HTMLElement>(".panel-body");
  if (previousPanel) panelScroll = previousPanel.scrollTop;
...
<div class="map-surface ${paused ? "paused" : ""}"><div class="map-canvas" style="transform:translate(${mapPanX}px,${mapPanY}px) scale(${mapZoom * BASE_MAP_SCALE})">${mapMarkup()}</div><div class="map-tools" aria-label="Nástroje mapy">${button("Kurzor", "mapmode:select", false, mapMode === "select" ? "active" : "")}${button("↟ Vylepšit", "mapmode:upgrade", false, mapMode === "upgrade" ? "active" : "")}${button("⬟ Opevnit", "mapmode:fortify", false, mapMode === "fortify" ? "active" : "")}<span class="tool-divider"></span>${button("−", "zoom:out", false, "zoom-button")}${button(Math.round(mapZoom * 100) + " %", "zoom:reset", false, "zoom-reset")}${button("+", "zoom:in", false, "zoom-button")}</div><div class="zoom-hint">Kolečko: přiblížení · šipky: posun mapy</div><div class="map-score"><b>STÁTY</b>${game.nations.filter((f) => owned(game, f.id).length).map((f) => `<span><i style="background:${f.color}"></i>${owned(game, f.id).length} úz. · ⚔ ${num(f.army.infantry)}</span>`).join("")}</div>${ended ? `<div class="result-banner"><h2>${game.winner === game.player ? "Vítězství" : "Tvá frakce byla poražena"}</h2>${button("Nová kampaň", "new", false, "primary")}</div>` : ""}</div><div class="map-footer"><span>${mapMode === "upgrade" ? "Režim vylepšování: klikni na vlastní provincii" : mapMode === "fortify" ? "Režim opevňování: klikni na vlastní provincii" : "Mini jednotky trvale označují státní hranice"}</span><span>↑ vylepšení · ⬟ opevnění · ! přerušené zásobování</span></div><div class="notice" role="status">${esc(notice)}</div>
...
<aside><nav aria-label="Velitelský panel">${["region", "trade", "research", "operations", "diplomacy"].map((t) => button(({ region: "Území", trade: "Obchod", research: "Výzkum", operations: "Operace", diplomacy: "Vztahy" } as Record<string, string>)[t], "tab:" + t, false, tab === t ? "active" : "")).join("")}</nav><div class="panel-body">${tab === "region" ? regionPanel() : tab === "trade" ? tradePanel() : tab === "research" ? researchPanel() : tab === "operations" ? operationsPanel() : diplomacyPanel()}</div><div class="panel-bottom">Místní simulace · jedna národní armáda</div></aside></main>${dialog()}`;
  ...
  const nextPanel = document.querySelector<HTMLElement>(".panel-body");
  if (nextPanel) nextPanel.scrollTop = panelScroll;
}
...
app.addEventListener("click", (ev) => {
  ...
  if (region) {
    const id = Number(region.dataset.region), r = game.regions[id];
    if (mapMode === "upgrade" && r.owner === game.player)
      send({ type: "upgrade", region: id });
    else if (mapMode === "fortify" && r.owner === game.player)
      send({ type: "fortify", region: id });
    else if (mapMode !== "select" && r.owner !== game.player) {
      notice = "Tento nástroj lze použít jen na vlastní provincii.";
      render();
  ...
  if (kind === "upgrade") send({ type: "upgrade", region: target ?? selected });
    ...
    tab = value;
    panelScroll = 0;
  } else if (kind === "mapmode") {
    mapMode = value as "select" | "upgrade" | "fortify";
    notice = mapMode === "upgrade" ? "Kliknutím na vlastní provincii ji vylepšíš." : mapMode === "fortify" ? "Kliknutím na vlastní provincii zahájíš opevnění." : "Běžný režim výběru provincií.";
  } else if (kind === "zoom") {
    if (value === "reset") { mapZoom = 1; mapPanX = -150; mapPanY = -70; }
    else mapZoom = Math.min(2.4, Math.max(.8, mapZoom * (value === "in" ? 1.18 : .85)));
  }
  else if (kind === "save") {
    save();
    notice = saveStatus;
...
document.addEventListener("keydown", (ev) => {
  ...
  if (["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(ev.key)) {
    ...
    mapPanY = Math.max(-2200, Math.min(900, mapPanY));
    render();
    return;
  }
  if (ev.code === "Space") {
